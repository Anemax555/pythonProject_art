...
def article_get(t):
    try:
        ...
        if (len(f_title) == 0):
            f_title = tree.xpath("//div[@class='content clearfix']/div[@class='box_left3']/div[@class='xiangxi']/h2/text()")
        ...
        if len(f_title) == 0:
            print("版面异常：" ,url)
            return
        f_context_zz = re.compile(r'<div class="txt_con" id="ctrlfscont".*?>(?P<context>.*?)</div>',re.S)
        ...
        for i in range(0, len(img_url)):
            imgfname = f_inputTime[0:10] + news_id + "_" + str(i) + os.path.splitext(img_url[i])[1]
            f_context = f_context.replace(img_url[i], "http://hzlaiqian.com/media/img/" + imgfname)
        ...
        img_list = ','.join(img_url)
        params = (news_id, f_title, f_context, f_source, f_sourceTime, url, f_inputTime, img_list, "证券时报")
        # print(params)
        input_mysql(params)
    except:
        print("Erro ",url)
...
#
# url = ['https://news.stcn.com/news/202207/t20220702_4713984.html']
# page_index_get(url)
